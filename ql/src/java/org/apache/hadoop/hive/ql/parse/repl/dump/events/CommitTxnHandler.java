...
class CommitTxnHandler extends AbstractEventHandler {
  ...
  public void handle(Context withinContext) throws Exception {
    ...
    if (!withinContext.hiveConf.getBoolVar(HiveConf.ConfVars.REPL_DUMP_METADATA_ONLY)) {
      ...
      WriteEventInfo writeEventInfo = HiveMetaStore.HMSHandler.getMSForConf(withinContext.hiveConf).
              getAllWriteEventInfo(commitTxnMessage.getTxnId());
      if (writeEventInfo != null && writeEventInfo.getTables() != null && writeEventInfo.getTables().size() > 0) {
        commitTxnMessage.addWriteEventInfo(writeEventInfo.getDatabases(), writeEventInfo.getTables(),
                writeEventInfo.getPartitions(), writeEventInfo.getWriteIds(), writeEventInfo.getTableObjs(),
                writeEventInfo.getPartitionObjs(), writeEventInfo.getFiles());
        payload = commitTxnMessage.toString();
      }

      org.apache.hadoop.hive.ql.metadata.Table qlMdTable;
      int numEntry = (commitTxnMessage.getTables() != null ? commitTxnMessage.getTables().size() : 0);
      ...
      for (int idx = 0; idx < numEntry; idx++) {
        ...
        if (!qlMdTablePrev.getTableName().equals(qlMdTable.getTableName())) {
          InsertHandler.createDumpFile(context, qlMdTablePrev, qlPtns, false, filesTobeAdded);
          qlPtns = new ArrayList<>();
          filesTobeAdded = new ArrayList<>();
          qlMdTablePrev = qlMdTable;
        }

        if (qlMdTable.isPartitioned() && (null != commitTxnMessage.getPtnObj(idx))) {
          qlPtns.add(new org.apache.hadoop.hive.ql.metadata.Partition(qlMdTable, commitTxnMessage.getPtnObj(idx)));
        }

        String[] fileList = commitTxnMessage.getFiles(idx).split(",");
        filesTobeAdded.add(Lists.newArrayList(fileList));
      }
      if (qlMdTablePrev != null) {
        InsertHandler.createDumpFile(context, qlMdTablePrev, qlPtns, false, filesTobeAdded);
      }
